...
class LessonItem:
    ...
    attr_type = {'title': str, 'practices': int, 'duration': int}
    ...
    def __setattr__(self, key, value):
        if key in self.attr_type and type(value) is self.attr_type[key]:
            if key == 'title' or 0 < value:
    ...
    def __delattr__(self, item):
        if item in self.attr_type.keys():
            return False
...
# Создаем наш первый модуль - экземпляр класса Module


module_1 = Module("Часть первая")

# Добавляем необходимые уроки (экз. класса LessonItem) в наш модуль
...
# Создаем новый модуль и добавляем в него необходимые уроки
